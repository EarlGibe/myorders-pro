...
function populateArticoli(articolo){

    const articoliList = document.getElementById("articoliList");
    // Popolamento dell'elenco delle articolo
   
        const listItem = document.createElement("li");
        listItem.textContent = articolo.nome;
      
        const selectButton = document.createElement("button");
        selectButton.textContent = "Seleziona";
        selectButton.addEventListener("click", () => {
          // Logica per gestire la selezione dell'catalogo
            console.log("Articolo selezionato:", articolo._id);
            localStorage.setItem("articoloSelezionato",JSON.stringify(articolo))
            //window.location.href='./articoliDisponibili.html';
         
        });
      
        listItem.appendChild(selectButton);
        articoliList.appendChild(listItem);
